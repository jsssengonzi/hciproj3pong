...
intro_text = """
    Hi welcome to hands free pong! 
    Here are the instructions to play. 
    Say start to play and pause to go back to the menu. 
    When you are in the menu, you can switch in between three game modes, easy, hard, and insane, by saying easy, hard or insane. 
    The ball movement will be represented with tones that go up and down with the ball and change volume as they get closer to you. 
    Listen carefully. 
    When the ball hits a paddle, a snare sound will be made. 
    When the ball hits the top or bottom, a tom drum will be made. 
    When a point is scored, a buzz noise will be made and the score will be read out. 
    Player one is the first number read and player two is the second number read. 
    In the game, you can move your paddle by singing up and down or stating a position between 0 and 450.  
    Play smart: while stating a position will increase granularity of movement, it will be a slower reaction. 
    Good luck, start when you are ready!
"""
...
if __name__ == '__main__':
    ...
    intro = gTTS(text=intro_text, lang="en", slow=False)
...
# for playing a fixed frequency sound
...
def generate_sine_wave(frequency, duration, volume=0.2, sample_rate=44100):
    ...
    stream = pa.open(
        ...
        channels=1,  # mono
        rate=sample_rate,
        frames_per_buffer=1,  # change to 2048?
        output=True,
    )
...
def hit():
    playsound('tom1-short.aiff')

# sound for puck hit


def hitpuck():
    playsound('snare.aiff')
...
def on_receive_ball(address, *args):
    ...
    if fr % 16 == 0:
        hz = ((args[1] / ((200 / 100)) + 100) // 10) * 10
        file = "freq/" + str(int(hz)) + "hz.wav"
        print(file)
        playsound(file)
    pass

    # use volume to show distance away from you and pitch to show height - done
    print("> ball position: (" + str(args[0]) + ", " + str(args[1]) + ")")
    pass


def on_receive_paddle(address, *args):
    # maybe tell where the position of the paddle is on hit, this is too much info for a fast paced game though
...
tts("Hi welcome to hands free pong. Here are the instructions to play. Say start to start and pause to pause. Switch between three game modes, easy, hard, and insane by saying")
...
def on_receive_scores(address, *args):
    # can you add who is winning the point?
    # if left != args[0]:
    #     tts("left scored")
    # if right != args[1]:
    #     tts("right scored")
...
def listen_to_speech():
    ...
    while not quit:
        ...
        try:
            ...
            # set level
            if recog_results == "easy mode" or recog_results == "easy":
                client.send_message('/l', 1)
                tts("Easy mode")

            if recog_results == "hard mode" or recog_results == "hard":
                client.send_message('/l', 2)
                tts("hard mode")

            if recog_results == "insane mode" or recog_results == "insane":
                client.send_message('/l', 3)
                tts("insane mode")

            # alter the height of a puck
            # user can set to a specific height between 0 and 450 by saying it or generically put the puck in the top, bottom, or middle based on the sound they hear
            if recog_results == "top":
                client.send_message('/p', 150)
            if recog_results == "middle":
                client.send_message('/p', 225)
            if recog_results == "bottom":
                client.send_message('/p', 350)

            try:
                if int(recog_results) > 0 or int(recog_results) < 450:
                    client.send_message('/p', int(recog_results))
            except:
...
def sense_microphone():
    ...
    while not quit:
        ...
        #####
        pos = position(pitch)
        if pos > 0 and pos < 450:
            client.send_message('/p', pos)
        # client.send_message('/l', volume)

        # uncomment these lines if you want pitch or volume
        if debug:
            print("pitch "+str(pitch)+" volume "+str(volume))
    # keep track of changes pretty early
    # return pitch
# -------------------------------------#
...
# -------------- Ball position: you can find it here -------
    ...
    def update_ball(self):
        ...
        # towards_me = (b.x < b.x_old) #
        # if towards_me:
        #     #partition board by y coordinate and assign pitch by ranges
        #     vol = 0.1
        #     for i in range(1, 16):
        #         if (30 * i - 18) < b.y < (30 * i - 12):
        #             generate_sine_wave(1800 - (50 * i), 0.7, volume= vol)
        #             #adjust volume by y increase
        #             vol = vol + 0.02
        #             break
    ...
    def update(self):
        ...
        if self.menu_key in pks:
            self.toggle_menu()
            # debounce: get rid of quick duplicated presses
...
class View(object):
    ...
    def redraw_menu(self):
        ...
        self.start_label = pyglet.text.Label("press space to start", font_name='San Serif',
                                             font_size=36, x=self.w.width//2, y=self.w.height//2, anchor_x='center', anchor_y='center')
        self.level_label = pyglet.text.Label("easy | hard | insane", font_name='San Serif', font_size=24,
                                             x=self.w.width//2, y=self.w.height//2+100, anchor_x='center', anchor_y='center')
        if (self.m.level == 1):
            self.level_indicator_label = pyglet.text.Label(
                "------", font_name='San Serif', font_size=24, x=self.w.width//2-105, y=self.w.height//2+80, anchor_x='center', anchor_y='center')
        elif (self.m.level == 2):
            self.level_indicator_label = pyglet.text.Label(
                "------", font_name='San Serif', font_size=24, x=self.w.width//2-12, y=self.w.height//2+80, anchor_x='center', anchor_y='center')
        elif (self.m.level == 3):
            self.level_indicator_label = pyglet.text.Label(
                "---------", font_name='San Serif', font_size=24, x=self.w.width//2+92, y=self.w.height//2+80, anchor_x='center', anchor_y='center')
...
class Window(pyglet.window.Window):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.score_label = pyglet.text.Label(str(p1_score)+':'+str(p2_score), font_name='San Serif',
                                             font_size=36, x=self.width//2, y=self.height//2, anchor_x='center', anchor_y='center')
...
if mode == 'host':
    # OSC thread
    # -------------------------------------#
    server_1 = osc_server.ThreadingOSCUDPServer(
        (host_ip, host_port_1), dispatcher_1)
    server_1_thread = threading.Thread(target=server_1.serve_forever)
    server_1_thread.daemon = True
    server_1_thread.start()
    server_2 = osc_server.ThreadingOSCUDPServer(
        (host_ip, host_port_2), dispatcher_2)
    ...
    print("> instruction: player 1 connects to " +
          str(host_port_1) + ", listen at "+str(player_1_port))
    print("> instruction: player 2 connects to " +
          str(host_port_2) + ", listen at "+str(player_2_port))
    # -------------------------------------#
...
if mode == 'player':
    # OSC thread
    # -------------------------------------#
    player_server = osc_server.ThreadingOSCUDPServer(
        (player_ip, player_port), dispatcher_player)
